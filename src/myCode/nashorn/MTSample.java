...
//		ScriptEngine s = factory.getScriptEngine("--optimistic-types=true");
		...
		// Test 5: MultiThread with P ScriptEngine and N Classes with Executor
		// Service
		System.out.println("\nMulti-threads with P ScriptEngine and N Classes with Executor Service.");
		System.out.println("========================================================================");
		Utils.timeMe();
		...
		Stack<ScriptEngine> ses=new Stack<ScriptEngine>();
		for (int i = 0; i < P; i++)
			ses.push(new ScriptEngineManager().getEngineByExtension("js"));
		...
		try {
			...
			while (!q.isEmpty()) {
				...
				{
					q.offerLast(fs);
//					long wt=(long) (Math.random()*10.0);
//					System.out.println("wait "+wt+"ms");
//					Thread.sleep(wt);
				}else
